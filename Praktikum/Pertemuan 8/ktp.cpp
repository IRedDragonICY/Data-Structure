...
void enqueue(Node** head, Data newData, long long int priority){
    Node* start = (*head);
    ...
    // Jika queue kosong atau newNode memiliki prioritas lebih tinggi
    if ((*head)->priority > priority) {
        newNode->next = *head;
        (*head) = newNode;
    } else {
        // Menelusuri list dan menemukan posisi untuk newNode
...
void dequeue() {
    // menghapus berdasarkan priority terbesar
    if(head == nullptr) {
    ...
    // mencari node dengan prioritas tertinggi
    while(temp != nullptr) {
    ...
    // menghapus node dengan prioritas tertinggi
    if(maxNode == head) {
        head = head->next;
    } else {
    ...
    void input() {
        ...
        //  priority berdasarkan NIK
        newNode->priority = stoll(newNode->data.nik);
        enqueue(&head, newNode->data, newNode->priority);
        writeToFile(head);
    }
...
void sort() {
    ...
    do {
        ...
        while (ptr1->next != lptr) {
            if (ptr1->priority > ptr1->next->priority) {
                long long temp = ptr1->priority;
                ptr1->priority = ptr1->next->priority;
                ptr1->next->priority = temp;

                Data tempData = ptr1->data;
    ...
    void initialize() {
        ...
        cout << "Banyak Antrian KTP: " << count << endl;
        ...
        file.close();
        // memasukkan priority berdasarkan NIK
        Node* current = head;
        while (current != nullptr) {
            //  total digit NIK semuanya akan menjadi priority
            current->priority = std::stoll(current->data.nik);
            current = current->next;
        }
        sort();
    }
