def delArray(values, arrSize, posisi):    # baris 1
    ...
    for i in range(posisi, arrSize-1):    # baris 3 anggap for i=posisi, i<arrSize-1, i++
        print (f"3    for i={i}, {i}< {arrSize-1}, {i}++            #True")
        values[i] = values[i+1]           # baris 4
        print (f"4       values[{i}] = {values[i+1]}              {values}")
    print (f"3    for i={i+1}, {i+1}< {arrSize-1}, {i+1}++            #False")
